...
Por defecto, los intentos de intrusión se guardan localmente en el `localStorage` del navegador. Para enviar la información a un servidor de seguridad centralizado, edita el endpoint en `honeypot-logger.js`:
...
Puedes ver y gestionar los logs almacenados abriendo la consola del desarrollador (F12) y ejecutando:
*   `HoneypotLogger.getAllAttempts()` — Muestra en consola todos los intentos registrados.
*   `HoneypotLogger.exportAttempts()` — Descarga los datos recolectados en formato JSON.
*   `HoneypotLogger.clearAttempts()` — Limpia el historial local.

---

## ⚠️ Declaración de Uso
Este proyecto fue diseñado con fines educativos y de seguridad defensiva para la detección temprana de amenazas en servidores web. No realiza ninguna acción ofensiva ni almacena datos de usuarios legítimos que navegan por el portafolio.
